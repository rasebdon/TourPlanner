...
namespace TourPlanner.Server.BL.MapQuestAPI
{
    public class MapQuestMapService : MapQuestBase, IMapService
    {
        ...
        {
            Uri uri = new($"https://www.mapquestapi.com/staticmap/v5/map?key={_apiKey}&start={StrCoord(start.Latitude)}, {StrCoord(start.Longitude)}&end={StrCoord(end.Latitude)}, {StrCoord(end.Longitude)}");
